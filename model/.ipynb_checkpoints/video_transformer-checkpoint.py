...
class VideoPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768,
        ...
        num_patches = (img_size[1] // patch_size[1]) * \
            (img_size[0] // patch_size[0]) * num_frames
...
class SpaceTimeBlock(nn.Module):
    ...
    def forward(self, x, einops_from_space, einops_to_space, einops_from_time, einops_to_time,
        ...
        x = space_residual + \
            self.drop_path(self.mlp(self.norm2(space_residual)))
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.clustering = video_params['clustering']
    ...
    def forward_features(self, x, text_emb, epoch=0):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i == len(self.blocks)-1:  # and self.training:
                ...
                if self.clustering:
                    vd_inputs = x.clone()
                    vd_outputs, encoding_indices = self.SoftRegion(
                        vd_inputs[:, 1:, :], epoch)
                    ...
                    if self.CLU_learn_region:
                        ...
                        x = torch.cat([vd_inputs[:, 0, :].unsqueeze(1), vd_outputs], dim=1)
        ...
        if self.temporal_type == 'late_fusion_fc':
            ...
            x = x.reshape(b, -1)
            # naive concat fusion
            x = self.late_temporal_fusion(x)  # [B, T*C] --> [B, C]
        elif self.temporal_type == 'late_fusion_pool':
            #  [BT, C] --> [B, T, C]
            x = x.reshape(b, f, -1)
            # print(b, f)
    ...
    def forward(self, x, text_emb, epoch=0):
        x = self.forward_features(x, text_emb, epoch)
        x = self.head(x)
        return x
