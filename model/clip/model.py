...
import traceback
import argparse
args = argparse.ArgumentParser(description='CLIP MODEL')
args.add_argument('--VQ_num_tokens', default=0, type=int)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        # print('CLIP Transformer:', args.VQ_num_tokens, width)
        self.width = width
        if args.VQ_num_tokens>0:
            self.RegionLearner = RegionLearner(VQ_num_tokens=args.VQ_num_tokens,
                                               VQ_token_dim=width,
                                               AGG_region_num=8,
                                               Interaction_depth=1,
                                               ME=False, # debug now
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.RegionLearner:
            # print('Before RL:', x.size()) # NLD
            x = self.go_RL(x)
            # print('After RL:', x.size())
        else:
    ...
    ###################################### ADDED BY MR. YAN ##########################################
    def go_RL(self, x, epoch=0, temporal_type='att', curr_frames = 1, random_sampling=False):
        b = x.size(0)
        for i, blk in enumerate(self.transformer.resblocks):
            ...
            if i == len(self.transformer.resblocks) - 2:
                ...
                if self.RegionLearner:
                    ...
                    if temporal_type == 'att':
                        vd_inputs = vd_inputs.reshape(b * curr_frames, -1, self.width)

                    vd_outputs, encoding_indices, region_mask = self.RegionLearner(vd_inputs, curr_frames, epoch)
                ...
                if random_sampling and self.training:
            ...
            if hasattr(l, name):
                ...
                if attr is not None:
                    ...
                    try:
                        ...
                        print("check it pls, maybe new layers", l, name, attr)
                        traceback.print_exc()
