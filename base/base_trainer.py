...
class Multi_BaseTrainer_dist:
    ...
    def __init__(self, args, model, loss, metrics, optimizer, config, writer=None, init_val=False):
        ...
        self.use_amp = cfg_trainer.get('use_amp', False)
        # self.debug = cfg_trainer.get('debug', False)
        self.accum_iter = cfg_trainer.get('accum_iter', 1)
        ...
        #  resume before DDP to avoid NCCL error, added by Mr. YAN
    ...
    def train(self):
        ...
        #################################################################################################
        print("Total epochs:\t", self.start_epoch, self.epochs + 1)
        for epoch in range(self.start_epoch, self.epochs + 1):
            result = self._train_epoch(epoch, scaler) # Automatic Mixed Precision added by Mr. Yan
            ...
            if self.mnt_mode != 'off' and self.args.rank == 0:
                ...
                if improved:
                    ...
                    not_improved_count = 0
                    best = True
                    self.logger.info("Best performance at {} epochs. ".format(epoch)) ## added by Mr. Yan for logging the best position
                else:
